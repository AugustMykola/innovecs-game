...
export class Spawner {
    ...
    constructor(
        ...
        private maxCount = 20,
        private minCount = 8,
        private burstPerTick = 3,
    ) {
        this.timer = this.nextInterval();
    }

    update(dt: number) {
        if (this.animals.length < this.minCount) {
            let spawned = 0;
            while (this.animals.length < this.minCount &&
            this.animals.length < this.maxCount &&
            spawned < this.burstPerTick) {
                this.spawnOne();
                spawned++;
            }
            // невеликий відкат таймера, щоб не було паузи після доливу
            this.timer = this.nextInterval() * 0.5;
            return;
        }

        if (this.animals.length >= this.maxCount) return;
        this.timer -= dt;
        if (this.timer <= 0) {
            this.spawnOne();
            this.timer = this.nextInterval();
